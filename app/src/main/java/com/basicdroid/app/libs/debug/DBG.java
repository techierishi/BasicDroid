package com.basicdroid.app.libs.debug;
...
public class DBG {
    ...
    static String debuggable[] = {"DebugTrace","LoginActivity","HomeActivity"};
     ...
     * @param color
     */
    public synchronized static void appendLog(String TAG,String text, boolean doAppend, COLOR color) {
        if (DEBUG) {
            if(TAG !=null && isDebuggable(TAG)) {
                DBG.d("appendLog", "" + text);
    ...
    public synchronized static boolean isDebuggable(String _tag){
        try {
            if(_tag !=null){
                for (String tag : debuggable) {
                    if(tag.equals(_tag)){
                        return true;
                    }
                }
